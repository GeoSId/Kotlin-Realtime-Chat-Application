...
import com.lkps.ctApp.utils.extension.saveFileTo
import java.io.File
...
object Utility {
    ...
    fun savePicTo(item: Message){
        Thread {
            val path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES + "/CT")
            if (!path.exists()) {
                path.mkdirs()
            }
            val imageFile = File(path, item.fileName + "." +item.fileExtension)
            item.fileUrl?.saveFileTo(imageFile)
        }.start()
    }

    fun savePdfTo(item: Message){
        Thread {
            val path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS + "/CT")
            if (!path.exists()) {
                path.mkdirs()
            }
            val imageFile = File(path, item.fileName + "." +item.fileExtension)
